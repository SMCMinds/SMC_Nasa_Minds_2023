...
for t in range(num_steps):
    ...
    for robot1 in robots:
        ...
        if robot.current_pos != robot.previous_pos:
            ...
            if robot.disabled_counter >= MAX_DISABLED_MOVES:
...
# not necessary
#if robots collide, If two rovers are about to collide while object avoiding, who goes first? We can choose the further rover, otherwise it might be arbitrary to choose one to go first.
pheromone_world5 = np.zeros((int(world_size), int(world_size)))
